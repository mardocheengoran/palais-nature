...
<html lang="en" xmlns:v="urn:schemas-microsoft-com:vml" xmlns:o="urn:schemas-microsoft-com:office:office">
    ...
    <body style="margin: 0; padding: 0; width: 100%; word-break: break-word; -webkit-font-smoothing: antialiased; --bg-opacity: 1; background-color: #eceff1; background-color: rgba(236, 239, 241, var(--bg-opacity)); font-family: 'Montserrat', Arial, sans-serif;">
        <div>
            ...
            @else
                <h1 style="text-align: center">Suivi de la commande N° {{ $invoice->code }} </h1><br>
            @endif
        </div>
                                    ...
                                    <table style="font-family: 'Montserrat', Arial, sans-serif; width: 100%;" width="100%" cellpadding="0" cellspacing="0" role="presentation">
                                        <tr>
                                            <td
                                                ...
                                                @if (in_array($role, ['admin', 'fournisseur'])and $invoice->state->id == 49)

                                                La commande N° {{ $invoice->code }} a été {{ $invoice->state->title }}.

                                                Le livreur en charge de cette commande est {{ $invoice->deliveryman->fullname }}.

                                                @elseif($role == 'livreur' and $invoice->state->id == 49)

                                                La commande N° {{ $invoice->code }} vous a été {{ $invoice->state->title }}. Vous êtes desormais en charge de l'acheminement de cette commande.

                                               @endif

                                               @if ($invoice->state->id != 49)
                                               La commande N° {{ $invoice->code }} a été {{ $invoice->state->title }}.
                                               @endif

                                               @if ($invoice->state->id == 50 and $role =='client')
                                                Le livreur en charge de votre commande vous la fera parvenir dans le plus bref delai.

                                               @endif

                                               @if ($invoice->state->id == 50 and $role =='fournisseur')
                                               Le livreur {{ $invoice->deliveryman->fullname }} en charge de cette commande l'a effectivement recuperé.

                                              @endif

                                               @if ($invoice->state->id == 51 and $role =='client' )
                                               {{ setting()->title }} vous remercie pour la confiance.
                                                À très bientôt !
                                               @endif
                                                ...
                                                @if (in_array($role, ['admin', 'livreur']))
                                                    <div style="border: 1px solid #ccc; margin-top: 20px; text-align: center; width: 100%;">
                                                        ...
                                                        <table class="table" style="width: 100%;">
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    @isset($invoice->address->subtitle)
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td style="font-family: 'Montserrat', Arial, sans-serif;">
                                                                <h3 style="font-weight: 700; font-size: 12px; margin-top: 0; text-align: right;">
                                                                    Passée {{ $invoice->created_at->formatLocalized('%A %d %B %Y à %H:%M') }}
                                                                </h3>
                                                            </td>
                                                        </tr>
                                                        <tr>
